...
def main(args=None):
    ...
    #The Order of movement for carrying cubes
    SEQUENCE = []
    for idx, (x, y, z) in enumerate(CUBE_POSE):
        back_home_seq = movement.back_home(x, y, z)
        if isinstance(back_home_seq, dict):
            back_home_seq = [back_home_seq]

        current_seq = (
            [
                movement.toward_to_object(x, y, z),
                movement.open_hand(),
                movement.approach_to_object(x, y, z), 
                movement.close_hand(),
                movement.lift_object(x, y, z),
            ]
            + movement.carry_object(x, y, z)
            + movement.placing_object(x, y, z)  
            + [
                movement.open_hand(),
            ]
            + movement.back_home(x, y, z) 
        )
